...
├── tests/                 239 tests, 95% coverage package-wide (100% on crypto, switch, and the webhook adapter) — includes property-based tests (Hypothesis) on the hand-rolled cryptography
...
**Doesn't exist at all yet:** a *hosted, multi-tenant* web UI — the reference UI generates private keys server-side, which is fine for local single-user use and explicitly not fine for a shared deployment (see spec section 14); an independent security audit (property-based testing and static analysis, both described in `SECURITY.md`, raise confidence in this particular implementation, but everything here has still been written and tested by its own author — a different bar than "reviewed by someone with no stake in it being correct"); and any legal opinion on whether trustee-quorum attestation has standing anywhere as evidence of death or incapacity. That last one specifically isn't something code can settle — it needs an actual lawyer, in an actual jurisdiction.
